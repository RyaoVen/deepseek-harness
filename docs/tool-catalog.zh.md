...
| `@deepseek-ai/dsh-tool-vibe-workflow` | `vibe` | `ctx.tools`、`ctx.workflowEngine`、`ctx.systemPrompt`、`a calling Agent (exec.agent parents the cluster children)` | `tool/call`、`tool/result` | - | vibe 运行一个 canned 集群脚本；只有 request/answers 与 spec 预填对模型可见，因此 schema 在角色提示词修改间保持稳定。 |
...
Spec 模式预填：在用户主导的设计讨论之后，传入已确认的产物让集群跳过对应阶段并一次跑完——"requirements"（需求档案：title、goal、requirements、assumptions）跳过产品经理，"design"（模块各含 id、title、layer、tasks、acceptance 的技术设计）跳过架构师，"styleGuide" 跳过 UI 设计师。工程师、测试工程师与升级仍然运行。
...
vibe 运行一个 canned 集群脚本；只有 request/answers 与 spec 预填对模型可见，因此 schema 在角色提示词修改间保持稳定。
