...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.hero.agentMode',
    kind: 'single',
    scope: 'root',
    summary: 'The session-mode chip beside the workspace picker on the new-session screen.',
    doc: 'The session-mode chip beside the workspace picker on the new-session\nscreen. Root scope: no session exists yet, so the choice is staged for\nthe next one and applied when it becomes current.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-mode-switcher ModeSeat',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.hero.agentMode\', () => ctx.slots.register(\n      { name: \'conversation.hero.agentMode\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-conversation ComposerAttach id \'composer-attach\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-mode-switcher ModeHeaderLabel id \'session-mode\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-desktop-decoration DecorationRow id \'desktop-decoration\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'extensions-center ExtensionsTab id \'extensions\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-usage-dashboard UsageDashboardSection id \'usage\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'root',
    summary: 'Desktop-shell title bar: the first row of the frame\'s column stack, rendered only when the page runs inside the frameless Electron shell (window.desktopBridge present).',
    doc: 'Desktop-shell title bar: the first row of the frame\'s column stack,\nrendered only when the page runs inside the frameless Electron shell\n(window.desktopBridge present). OCCUPIED by ui-desktop-decoration\'s\nTitleBar — the drag region and the window controls.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-desktop-decoration TitleBar id \'desktop-titlebar\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'shell.titlebar\', () => ctx.slots.register(\n      { name: \'shell.titlebar\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
