...
Modes are advisory: the `mode/set` event is `ignorable` for readers that do not know it, and a mode changes future behavior through the prompt section only. The shipped modes are `standard`, `creative`, `design`, `vibe`, and `spec`.
...
Spec mode is user-led: its guidance drives an interactive design discussion to module level first, then runs the same cluster once with the confirmed requirements and design prefilled.
