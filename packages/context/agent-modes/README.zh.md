...
模式是建议性的：`mode/set` 事件对不认识它的读者标记为 `ignorable`，模式只通过提示词段落改变未来行为。内置模式为 `standard`、`creative`、`design`、`vibe` 与 `spec`。
...
Spec 模式由用户主导：其指引先驱动交互式设计讨论到模块级，再用已确认的需求与设计预填一次跑完同一集群。
