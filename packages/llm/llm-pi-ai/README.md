...
A model entry may name its own `api`, winning over the route's: resolution goes entry `api` → route `api` → installed catalog entry → the protocol the route's shipped models agree on. Only two spellings are accepted — a protocol in `supportedProtocols()`, or the catalog model's own api, which restates the default and repoints nothing — and anything else fails resolution naming the route and model. A route on which any model's api diverges from the route-level answer is **repointed**: its models disagree on protocol, so it is served by a dispatching provider that resolves each request's implementation from the model's own api, and every model on such a route must speak a `supportedProtocols()` protocol — a catalog-only sibling (`bedrock-converse-stream`, say) fails resolution instead of silently leaving one model only the catalog provider could serve.
