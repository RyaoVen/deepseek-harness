...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/extensions/extensions-center': { kind: 'indirect', reason: 'The center mounts mcp-client fibers and writes skill files; dsh-mcp-client and the skill providers own every model-facing schema and catalog entry.' },
  ...
  'packages/client/ui-desktop-decoration': { kind: 'none', reason: 'Browser-side settings switch over the desktop shell bridge; registers nothing model-facing.' },
  ...
  'packages/host/usage-dashboard': { kind: 'none', reason: 'Host-side read-only usage fold over session logs; registers nothing model-facing.' },
